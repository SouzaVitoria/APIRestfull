...
class UserController {
    ...
    selectAll(){
        //let users = User.getUsersStorage(); //Pegar todas as informações de usuários cadastrados

        //Criar AJAX
        HttpRequest.get('/users').then(data=>{ //retornar uma promessa
                obj.users.forEach(dataUser=>{ //users é o array
                    let user = new User();
                    user.loadFromJSON(dataUser); //chamando nos getters e setters 
                    this.addLine(user); //Add as informações na tela
                });
        }); 
    }
